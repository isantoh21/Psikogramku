...
function normalizeMimeType(mimeType?: string, filename?: string, base64Data?: string): string {
...
async function startServer() {
  ...
  app.post("/api/extract-ist", async (req, res) => {
    try {
...
3. Jika data tertentu tidak ditemukan, beri nilai null atau string kosong "".`;
  ...
  app.post("/api/extract-kraepelin", async (req, res) => {
    try {
      if (!process.env.GEMINI_API_KEY) {
        return res.status(500).json({ error: 'GEMINI_API_KEY belum dikonfigurasi' });
      }
...
Jika data biodata tidak ditemukan, set string menjadi "". Jika data sikap kerja tidak ditemukan, set angka menjadi 0.`;
  ...
  app.post("/api/extract-papikostik", async (req, res) => {
    try {
      if (!process.env.GEMINI_API_KEY) {
        return res.status(500).json({ error: 'GEMINI_API_KEY belum dikonfigurasi' });
      }
...
Jika data tidak ditemukan, set string menjadi "" dan angka menjadi 0.`;
  ...
  app.post("/api/extract-mbti", async (req, res) => {
    try {
      if (!process.env.GEMINI_API_KEY) {
        return res.status(500).json({ error: 'GEMINI_API_KEY belum dikonfigurasi' });
      }
...
Jika data MBTI tidak ditemukan, kembalikan data kepribadian sebelumnya saja tanpa perubahan.`;
  ...
  app.post('/api/extract-bei', upload.single('file'), async (req, res) => {
    ...
    try {
      ...
      // 1. Jika DOCX, ekstrak teks dengan mammoth
      if (ext === '.docx') {
        try {
      ...
      // 3. Jika belum dapat teks (PDF, Audio, Gambar, atau format lainnya)
      if (!extractedText.trim()) {
        ...
        if (
          ...
          // Coba baca teks sebagai UTF-8 atau markitdown
          try {
...
Kembalikan HANYA format JSON valid persis seperti ini (tanpa markdown \`\`\`json):
{
  ...
  // MarkItDown Endpoint
  app.post('/api/markitdown', upload.array('files', 5), async (req, res) => {
        ...
        try {
          ...
          if (!markdownText) {
            try {
              ...
              // Gemini fallback for PDF or complex files
              if (process.env.GEMINI_API_KEY) {
                const fileBuffer = fs.readFileSync(file.path);
                const base64Data = fileBuffer.toString('base64');
                const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
